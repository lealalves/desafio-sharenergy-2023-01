<template>
    ...
    <div id="container-cat">
      ...
      <div id="inputs-container">
        <p>Selecione o código HTTP:</p>
        <select @change="changeImg($event)">
          <option>Selecione</option>
          <option v-for="status in statusArray" :value="status.status">{{status.status}}</option>
        </select>
      </div>
      <img v-if="this.actualStatus === 0" src="../assets/logo.jpg" alt="Gatinho">
      <img v-else id="cat-img" :src="`https://http.cat/${this.actualStatus}`" alt="Gatinho">
    </div>
	</main>
</template>
<style scoped>
  ...
  methods:{
    changeImg($event){
      if($event.target.value === 'Selecione'){
        return this.actualStatus = 0
      }

      this.actualStatus = $event.target.value
    }
  }
}
</script>